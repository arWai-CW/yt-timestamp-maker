...
function getStorageKeys(videoId: string): StorageKeys {
	return {
...
chrome.runtime.onMessage.addListener(
	(message: Message, sender, sendResponse) => {
		...
		const targetVideoId =
			videoId ?? (sender.tab?.url ? getVideoIdFromUrl(sender.tab.url) : null);
		...
		(async () => {
			switch (type) {
				case "getAllVideos": {
					...
					const videos: VideoInfo[] = Array.from(videoIds).map((vid) => ({
						videoId: vid,
						title: (allResults[`${STORAGE_KEY_TITLE}_${vid}`] as string) || null,
						url: `https://www.youtube.com/watch?v=${vid}`,
						count: ((allResults[`${STORAGE_KEY_LOGS}_${vid}`] as unknown[])?.length) || 0,
					}));
				...
				case "setVideoTitle": {
					...
					await chrome.storage.local.set({ [keys.title]: data as string });
					sendResponse({ success: true });
					break;
				}
				case "togglePanel": {
